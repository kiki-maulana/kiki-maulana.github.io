...
function slider(s_id) {
    ...
    $(inputRange).on('input', function () {
        ...
        let dataKML = $('#labels-range-2').find('.active.selected').data('kml')
        let dataKML2 = $('#labels-range-2').find('.active.selected').data('kml2')
        fechKML('map', dataKML, beforeMap)
        fechKML('map2', dataKML2, beforeMap2)
    });
